...
La politique d'exécution des commandes ne s'exprime **pas** en ligne de commande : le seul drapeau existant est `--dangerously-skip-permissions`, que Sillage n'utilise jamais. Elle vient de `~/.gemini/antigravity-cli/settings.json` (clé `toolPermission`, valeurs `always-proceed`, `request-review`, `strict`, `proceed-in-sandbox`). Avec le défaut `request-review`, une demande d'autorisation en mode print est auto-refusée et la session s'arrête aussitôt : stdout vide, sortie 0, explication sur stderr. Sillage lit ce fichier pour poser un avertissement de santé sur l'agent (voir SPEC-API.md §« Santé des agents ») qui demande `proceed-in-sandbox`, cohérent avec le `--sandbox` toujours forcé. Il ne l'écrit que sur clic explicite de l'humain (`POST /api/agents/{id}/fix-warning` → `fixAntigravityToolPermission` dans `store.go`) : relecture des clés existantes, une seule clé changée, écriture atomique (fichier temporaire + rename) en conservant les droits, et refus pur et simple si le JSON existant est illisible.

Même politique réglée, une confirmation reste possible : les lectures de fichiers dans le worktree passent sans règle, mais `.git` est traité comme un chemin sensible et exige un accord. Dans un worktree lié c'est un fichier d'une ligne, qu'un modèle en exploration essaie d'ouvrir, ce qui tue la session. `permissions.allow` n'accepte pas de motif de chemin pour ce cas (`read_file(<dossier>/*)` et `read_file(<racine>/**)` ne correspondent à rien, vérifié ; seul le chemin exact fonctionne, donc rien de réutilisable d'une tâche à l'autre) : l'adaptateur prévient l'agent dans son prompt (`antigravityWorktreeNote`) plutôt que d'élargir des autorisations.

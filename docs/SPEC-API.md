...
**Livraison constatée d'un chantier fusionné à la main.** Même chose un cran plus haut : une branche de chantier peut arriver dans sa destination sans passer par Sillage (`git merge` dans un terminal, pull request fusionnée sur la forge). `GET /api/cards/{id}/delivery` le constate au passage : une branche de chantier `shippedAt: null` entièrement contenue dans sa destination reçoit un `shippedAt`, avec l'événement SSE `cards`. Sans ça, seul `POST /api/cards/{id}/ship` pose `shippedAt` : la carte garderait `awaitingShip: true` et ne rejoindrait jamais la colonne `done`, alors que l'aperçu affiche déjà « Déjà sur `<destination>` ». Deux garde-fous : le chantier doit être livrable (`shipReady`, donc au moins une tâche acceptée) sinon une branche encore vide serait « déjà arrivée » par construction, et la branche doit être effectivement contenue dans la destination. Aucune écriture git, aucun `prUrl` inventé (celle déjà connue est conservée). La constatation a lieu **après** l'acceptation automatique ci-dessus : une tâche acceptée au passage peut rendre le chantier livrable, donc constatable comme déjà livré.
...
`GET /api/cards/{id}/delivery` (aucune écriture git ; peut accepter des tâches déjà fusionnées et constater un chantier déjà arrivé, voir ci-dessus) :
...
- La modale a deux temps dans le même cadre : un formulaire (précisions facultatives, dépôt à lire, agent), puis la proposition. **Jamais d'écran d'attente vide** : pendant l'aller-retour le bouton porte « Lecture du dépôt… » et une note dit que cela prend souvent une minute ou deux.

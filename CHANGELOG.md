...
- Homebrew users can keep Sillage running in the background, started at login: `brew services start sillage`. The formula sets an explicit `PATH` so agent CLIs (`claude`, `codex`, `gh`) stay visible to a service, which does not inherit your shell.

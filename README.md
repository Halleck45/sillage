...
```bash
go build -o atelier . && ./atelier
```

- Écoute par défaut sur `http://127.0.0.1:8787`.
- Au premier lancement, un mot de passe est généré et affiché une seule fois dans le terminal (hash stocké dans `~/.local/share/atelier/config.json`). Pour le changer : relancer avec `ATELIER_PASSWORD=nouveau ./atelier` (le hash est remplacé).
- Données : `~/.local/share/atelier/state.json` (écriture atomique). Worktrees git des tâches : `~/.local/share/atelier/worktrees/`.

## Accès distant (téléphone)

Le serveur ne parle que HTTP : ne jamais l'exposer nu sur Internet. Deux options sûres :

1. **Tailscale** (recommandé) : `./atelier -addr 100.x.y.z:8787` (ou `-addr 0.0.0.0:8787` si le pare-feu ne laisse passer que tailscale0), puis ouvrir l'URL Tailscale depuis le téléphone. Chiffré de bout en bout, zéro config TLS.
...
- **Validation humaine obligatoire** : les agents travaillent dans un worktree git isolé, sur une branche `atelier/<ref>-...`. Aucun agent n'a le droit de pousser. Le `git push` n'existe qu'à un seul endroit du code et n'est déclenché que par le bouton « Pousser et livrer » avec confirmation explicite.

...
Note for Antigravity agents: `agy` asks for approval before every terminal command, and headless nobody can grant it, so the run stops without producing anything. Its execution policy lives in `~/.gemini/antigravity-cli/settings.json`, not in a flag; add `"toolPermission": "proceed-in-sandbox"` there and commands run without prompting, but only inside the terminal sandbox that Sillage always forces (`--sandbox`). Sillage never passes the permission-bypass flag; until the policy is set, the agent carries a warning that spells out the fix and offers a button to write that one key for you (your other settings are preserved, and an unreadable file is never overwritten).
